...
const CONTRACT_ADDRESS = 'YOUR_CONTRACT_ADDRESS'; // Contract ID in 0.0.xxxxx format
const CONTRACT_ABI = [
...
// Replace with your Hedera account credentials
const ACCOUNT_ID = 'YOUR_ACCOUNT_ID'; // Account ID in 0.0.xxxxx format
  ...
  useEffect(() => {
    const initializeContract = async () => {
      try {
        ...
        const hethersContract = new Contract(contractId, CONTRACT_ABI, signer);
    ...
    const handleMint = async () => {
        ...
        try {
            ...
            const tx = await contract.mint(ACCOUNT_ID, amount);
            await tx.wait();

            setSuccessMessage('Tokens minted successfully!');
            const newTotalSupply = await contract.totalSupply();
            const newBalance = await contract.balanceOf(ACCOUNT_ID);
